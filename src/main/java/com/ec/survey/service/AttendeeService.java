...
public class AttendeeService extends BasicService {
	...
	public int getNumberOfAttendees(Integer ownerId, Map<String, String> attributeFilter) throws Exception {
		...
		NativeQuery query = session.createNativeQuery("SELECT count(*) " + sql);
		sqlQueryService.setParameters(query, parameters);
	...
	public List<Integer> getAccessibleAttendees(Integer ownerId, Map<String, String> attributeFilter) throws Exception {
		...
		NativeQuery query = session.createNativeQuery("SELECT a.ATTENDEE_ID " + sql);
		sqlQueryService.setParameters(query, parameters);
	...
	public void add(Attendee attendee) {
		...
		if (attendee.getOriginalId() != null) {
			Query query = session.createNativeQuery("UPDATE SHARES_ATTENDEE SET attendees_ATTENDEE_ID = :id WHERE attendees_ATTENDEE_ID = :origId")
	...
	public void delete(List<Attendee> attendees) {
		...
		Query query = session.createNativeQuery("DELETE FROM PARTICIPANTS_ATTENDEE WHERE attendees_ATTENDEE_ID IN (:ids)")
				.setParameterList("ids", ids);
		query.executeUpdate();

		query = session.createNativeQuery("DELETE FROM SHARES_ATTENDEE WHERE attendees_ATTENDEE_ID IN (:ids)")
				.setParameterList("ids", ids);
		query.executeUpdate();
