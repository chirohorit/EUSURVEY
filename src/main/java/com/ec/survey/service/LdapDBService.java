...
public class LdapDBService extends BasicService {
	...
	public void reload(Set<DepartmentItem> departments) {
		...
		NativeQuery query = session.createNativeQuery("SELECT DISTINCT DOMAIN_CODE domainCode, NAME name FROM DEPARTMENTS");
	...
	public List<String> getDomainKeySufixes() {
		...
		Session session = sessionFactory.getCurrentSession();		
		Query query = session.createNativeQuery( "select REPLACE(DOMAINS.CODE ,'eu.europa.','') code_sufix  from DOMAINS ");
		@SuppressWarnings("unchecked")
