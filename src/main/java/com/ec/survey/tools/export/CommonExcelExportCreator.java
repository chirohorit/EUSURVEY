...
public abstract class CommonExcelExportCreator extends ExportCreator {
			...
			if ((publication != null || filter.exported(question.getId().toString()))
				...
				if (question instanceof Matrix) {
					...
					for (Element matrixQuestion : matrix.getQuestions()) {
						...
						if (export != null && export.getSplitMCQ() && !matrix.getIsSingleChoice()) {
							for(var answer : matrix.getAnswers()) {
								cell = rowInsertHeader.createCell(columnIndexInsertHeader++);
								cellValue = answer.getStrippedTitleNoEscape2();
								if (export.getShowShortnames()) {
									cellValue += " (" + answer.getShortname() + ")";
								}
								cell.setCellValue(cellValue);
								sheetInsertHeader.setColumnWidth(columnIndexInsertHeader, 5000);
								checkColumnInsertHeader(export);
					...
					for (ComplexTableItem childQuestion : table.getQuestionChildElements()) {
						...
						if (export != null && export.getSplitMCQ() && childQuestion.getCellType() == ComplexTableItem.CellType.MultipleChoice) {
							for (var pa : childQuestion.getPossibleAnswers()) {
								cell = rowInsertHeader.createCell(columnIndexInsertHeader++);
								cellValue = pa.getStrippedTitleNoEscape2();
								if (export.getShowShortnames()) {
									cellValue += " (" + pa.getShortname() + ")";
								}
								cell.setCellValue(cellValue);
								sheetInsertHeader.setColumnWidth(columnIndexInsertHeader, 5000);
								checkColumnInsertHeader(export);
					...
					if (export != null && export.getSplitMCQ() && question instanceof MultipleChoiceQuestion) {
					...
					} else if (export != null && export.getSplitMCQ() && question instanceof GalleryQuestion) {
						var gallery = (GalleryQuestion) question;
						if (gallery.getSelection()) {
							for (var file : gallery.getFiles()) {
								cell = rowInsertHeader.createCell(columnIndexInsertHeader++);
	...
	public void exportContent(Publication publication, boolean sync, ResultFilter resultFilter) throws Exception {
		...
		if (answersets != null) {
			...
			for (String attrib : values.keySet()) {
				...
				if (value instanceof String) {
					...
					query.setParameter(attrib, (Date) values.get(attrib));  // Timestamp
				}
			}
		...
		if (fileCounter > 0 || !uploadedFiles.isEmpty() || explanationFilesToExport.hasFiles()) {
			...
			if (publication != null) {
				os.putArchiveEntry(new ZipArchiveEntry(FilenameUtils.getName(exportFilePath + getFileExtension())));
			} else {
			...
			if ((publication != null || filter.exported(question.getId().toString()))
				...
				if (question instanceof Matrix) {
					...
					for (Element matrixQuestion : matrix.getQuestions()) {
						...
						if (answerSet == null) {
							...
							if (v != null) {
								if (export != null && export.getSplitMCQ() && !matrix.getIsSingleChoice()) {
									...
									v += "; ";
									for (var pa : matrix.getAnswers()) {
										cell = checkColumnsParseAnswerSet();
										if (v.contains(getPACheckName(pa))) {
						...
						} else if (export != null && export.getSplitMCQ() && !matrix.getIsSingleChoice()) {
							List<Answer> answers = answerSet.getAnswers(matrixQuestion.getUniqueId());
							for (var pa : matrix.getAnswers()) {
								cell = checkColumnsParseAnswerSet();
								if (answers.stream().anyMatch(answer -> pa.getUniqueId().equals(answer.getPossibleAnswerUniqueId()) || pa.getId().equals(answer.getPossibleAnswerId()))) {
									cell.setCellValue("Yes");
								} else {
									cell.setCellValue("No");
								}
					...
					for (ComplexTableItem childQuestion : table.getQuestionChildElements()) {
						...
						} else if (export != null && export.getSplitMCQ() && childQuestion.getCellType() == ComplexTableItem.CellType.MultipleChoice) {
							List<Answer> answers = answerSet.getAnswers(childQuestion.getUniqueId());
							for (var pa : childQuestion.getPossibleAnswers()) {
								cell = checkColumnsParseAnswerSet();
								if (answers.stream().anyMatch(answer -> pa.getUniqueId().equals(answer.getPossibleAnswerUniqueId()) || pa.getId().equals(answer.getPossibleAnswerId()))) {
					...
					if (answerSet == null) {
						...
						if (v != null) {
							if (export != null && export.getSplitMCQ() && gallery.getSelection()) {
								v += "; ";
								for (var file : gallery.getFiles()) {
									cell = checkColumnsParseAnswerSet();
									if (v.contains(file.getName() + "; ")) {
										cell.setCellValue("Yes");
									} else {
										cell.setCellValue("No");
									}
					...
					} else if (export != null && export.getSplitMCQ() && gallery.getSelection()) {
						List<Answer> answers = answerSet.getAnswers(question.getUniqueId());
						for (int i = 0; i < gallery.getFiles().size(); i++) {
							var file = gallery.getFiles().get(i);
							cell = checkColumnsParseAnswerSet();
							var fileValue = Integer.toString(i);
							if (answers.stream().anyMatch(answer -> file.getUid().equals(answer.getPossibleAnswerUniqueId()) || fileValue.equals(answer.getValue()))) {
								cell.setCellValue("Yes");
							} else {
								cell.setCellValue("No");
							}
	...
	private String getPACheckName(Element possibleAnswer) {
		var checkName = possibleAnswer.getStrippedTitleNoEscape2();
		if (export != null && export.getShowShortnames()) {
			checkName += "(" + possibleAnswer.getShortname() + ")";
		}
		checkName += "; ";
		return checkName;
	}
	...
	private void linkCell(final Cell cell, final String linkValue, final CreationHelper createHelper) {
		var link = createHelper.createHyperlink(HyperlinkType.FILE);
