...
<head>
	...
	function updateFileList(element, responseJSON, doNotRemove=false) {
		...
		for (var i = 0; i < responseJSON.files.length; i++) {
			...
			$(del).attr("type", "button").addClass("unstyledbutton").attr("data-toggle","tooltip").attr("title","${form.getMessage("label.RemoveUploadedFile")}").attr("aria-label",f).attr(
					"onclick",
				...
				<c:if test="${form.survey.contact != null}">
					...
					<c:choose>
						<c:when test="${form.survey.contact.startsWith('form:')}">
							<a class="link visibleLink" target="_blank" data-toggle="tooltip" title="<spring:message code="info.ContactForm" />" aria-label="<spring:message code="info.ContactForm" />" href="${serverprefix}runner/contactform/${form.survey.shortname}"><spring:message code="label.ContactForm" /></a>
						</c:when>
